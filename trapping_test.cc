...
void GenericTestOperatorAdd() {
  ...
  }
  {
    ...
    EXPECT(x == numeric_limits<T>::min() + 1);
    x = numeric_limits<T>::min();
    EXPECT((x + 1) == T{numeric_limits<T>::min() + 1});
    EXPECT((x + 1) == numeric_limits<T>::min() + 1);
    EXPECT((1 + x) == T{numeric_limits<T>::min() + 1});
    EXPECT((1 + x) == numeric_limits<T>::min() + 1);
  }
}
...
void TestOperatorOr() {
  {
    trapping<u32> x{0x7fffffff};
    constexpr u32 high_bit = 0x80000000;
    EXPECT((x | high_bit) == 0xffffffff);
    EXPECT((high_bit | x) == 0xffffffff);
  }
  {
    trapping<u32> good{0x600d0000};
    constexpr u32 cafe = 0xcafe;
    EXPECT((good | cafe) == u32{0x600dcafe});
    EXPECT((cafe | good) == u32{0x600dcafe});
  }
}
...
void TestOperatorXor() {
  {
    trapping<u32> x{0xffffffff};
    constexpr u32 high_bit = 0x80000000;
    EXPECT((x ^ high_bit) == u32{0x7fffffff});
    EXPECT((high_bit ^ x) == u32{0x7fffffff});
  }
  {
    trapping<u32> good{0x600d0000};
    constexpr u32 cafe = 0xcafe;
